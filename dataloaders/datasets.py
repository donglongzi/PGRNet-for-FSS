...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # sample['original_img'] = torch.from_numpy(original_img[idx])
    ...
    def getSupport(self, label=None, all_slices=True, N=None):
        ...
        #原来下面三个赋值操作是专门针对CMR数据集中标签（取值为0， 200， 500， 600）所设计的，将其映射为1， 2， 3
        lbl[lbl == 200] = 1
        lbl[lbl == 500] = 2
        lbl[lbl == 600] = 3
        lbl = 1 * (lbl == label)
        ...
        if all_slices:
            ...
            idx = lbl.sum(axis=(1, 2)) > 0  #先利用这个index将包含目标类别的所有切片取出
            idx_ = self.get_support_index(N, idx.sum())  #再分别取出三个子块中的中间切片作为支持slice来指导分割

            sample['image'] = torch.from_numpy(img[idx][idx_]) #先利用这个index将包含目标类别的所有切片取出, 再分别取出三个子块中的中间切片作为支持slice来指导分割
